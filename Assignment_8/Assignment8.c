    ...
    while(instr[i]!='f'){
        ...
        switch (instr[j]){
            case '0':
                   ...
                   reg[convert(instr[j+1])]=reg[convert(instr[j+2])] + reg[convert(instr[j+3])];
            ...
            case '3':
                   ...
                   reg[convert(instr[j+1])]=reg[convert(instr[j+1])] + 1;
                   //find_col is used to find the pos where we need to keep decode,and execute,mem,writeback
                   find_col=false;
                   k=1;
                   //until we find a column,we go to the next col to find it.
            ...
            case '6':
                   ...
                   reg[convert(instr[j+1])]=~reg[convert(instr[j+2])];
                   //find_col is used to find the pos where we need to keep decode,and execute,mem,writeback
                   find_col=false;
                   k=1;
                   //until we find a column,we go to the next col to find it.
     ...
     halt_count++;  //incrementing halt and total instr count by 1 as we didn't count halt instr from above loop
       ...
       FILE* fptr;
     fptr=fopen("DcacheOutput.txt","w");   // to open the file Dcache.txt to write our new data values in the given format
    i=0;
    while(i<512){
        char s[3];
        s[0]=data[i];
